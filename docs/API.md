...
Согласованного ящика деканата/приёмной комиссии нет, поэтому реально письмо всегда уходит на
один и тот же корпоративный ящик — `sent_to` равен `SUPPORT_EMAIL`, который по умолчанию равен
`MAIL_FROM` (см. `.env.example`): ящик пишет сам себе, а адресат по сути — в теме и в первых
строках тела письма («Кому», «Номер обращения», «Приоритет», «Заявитель»). Если `anonymous:
false` и почта заявителя подтверждена, письмо уходит с заголовком `Reply-To` на эту почту —
сотрудник отвечает прямо из письма; для анонимного обращения `Reply-To` не ставится.
